...
class PrintOutputDefault(PrintOutput):

    def make_output(self, info):
        print(info)
...
def main():
    book = load_data()
    outputter = PrintOutputDefault()
    outputter.make_output("Welcome to the assistant bot!")
    while True:
        outputter.make_output("Enter a command: ")
        user_input = input()
        command, *args = parse_input(user_input)

        if command in ["close", "exit"]:
            outputter.make_output("Good bye!")
            with open(file_path, "wb") as file:
                pickle.dump(book, file)
            break

        elif command == "hello":
            outputter.make_output("How can I help you?")

        elif command == "add":
            outputter.make_output(add_contact(args, book))

        elif command == "change":
            outputter.make_output(change_contact(args, book))

        elif command == "phone":
            outputter.make_output(show_phone(args, book))

        elif command == "all":
            outputter.make_output(show_all(book))

        elif command == "add-birthday":
            outputter.make_output(add_birthday(args, book))

        elif command == "show-birthday":
            outputter.make_output(show_birthday(args, book))

        elif command == "birthdays":
            birthdays = book.get_upcoming_birthdays()
            if not len(birthdays):
                outputter.make_output("There are no upcoming birthdays.")
                continue
            for day in birthdays:
                outputter.make_output(f"{day}")

        else:
            outputter.make_output("Invalid command.")
